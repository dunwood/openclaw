...
services:
  - type: web
    ...
    plan: free
    dockerCommand: >-
      sh -lc 'node openclaw.mjs config set gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback true --strict-json && node openclaw.mjs gateway --allow-unconfigured --bind lan --port 8080'
...
| `healthCheckPath`     | Render monitors `/healthz` (alias: `/health`) for health checks |
| `sync: false`         | Prompts for value during deploy (secrets)                       |
| `generateValue: true` | Auto-generates a cryptographically secure value                 |
| `dockerCommand`       | Binds OpenClaw to Render's expected public port (`8080`)        |
...
The Blueprint defaults to `free`. Free tier has no persistent disk, so config resets on each redeploy.
Upgrade to Starter (or above) and mount a disk if you need persistent state.
...
- Port mismatch — Render requires your app to listen on `0.0.0.0:8080`; keep
  `dockerCommand`/`OPENCLAW_GATEWAY_PORT` aligned with `PORT=8080`
- Control UI startup guard — for non-loopback bind, OpenClaw requires Control UI origin policy.
  The Blueprint `dockerCommand` pre-sets `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` before gateway start.
...
Render expects a 200 response from `/healthz` within 30 seconds. (`/health` is also supported.) If builds succeed but deploys fail, the service may be taking too long to start. Check:
